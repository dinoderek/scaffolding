...
11. In `session-recorder`, logged sets render as in-card rows with a header row (`Type`, `Weight`, `Reps`) instead of per-set subcards, and set numeric validation uses visual cues only (no inline validation text):
    ...
    - Logging a new exercise focuses its first `Weight` input, and adding a set focuses the new set's `Weight` input. Weight inputs select their current value on focus so copied/defaulted values can be overwritten quickly.
