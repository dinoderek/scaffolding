...
Decide and lock the backend platform for MVP, stand up a minimal backend that runs locally, implement secure user authentication/authorization, and expose authenticated sync APIs for `gyms`, `sessions`, `session_exercises`, and `exercise_sets`.
...
3. Secure auth/authz baseline implemented and verified.
4. Sync API endpoints for session-tracking entities with ownership checks.
5. Backend contract test suite covering auth, authz, and API behavior.
6. Updated AI/testing/task-template guidance capturing backend project structure and testing expectations.
7. Standard local quality-gate command path/checklist for fast repeatable verification (with current no-CI posture documented).
...
4. Authentication works for MVP-required flows and issues verifiable identity for API access.
5. A user provisioning mechanism exists for this stage so new users can be added without FE integration.
6. Authorization prevents cross-user data access for all sync entities; negative tests prove unauthorized access is rejected.
7. API supports authenticated read/write operations for `gyms`, `sessions`, `session_exercises`, and `exercise_sets`.
8. FE integration is explicitly deferred, but API contract documentation is sufficient for a later integration task.
9. Backend quality gates and contract tests defined by the milestone task cards pass before milestone closeout.
10. `docs/specs/04-ai-development-playbook.md`, `docs/specs/06-testing-strategy.md`, and task template(s) are updated to reflect backend testing layers, manual verification posture (no CI yet), and project-structure conventions.
11. A standard local quality-gate command path (or documented wrapper set) is defined for fast checks, with clear scope and limits (what it covers vs what remains manual/task-specific).
...
3. `docs/tasks/complete/T-20260220-09-m5-backend-deployment-strategy-and-environments.md` - originally planned deployment strategy task; later marked outdated and deferred to a future cloud-environment milestone. (`outdated`)
...
2. Cloud deployment/environment strategy is deferred to a later milestone after frontend sync integration clarifies the required hosted-operational shape. (`planned`)
...
- Hosted environment strategy remains deferred and should not be treated as part of M5 closeout anymore.
...
- Date: 2026-03-02
- Decision: Remove cloud deployment/environment strategy from M5 closeout and defer it to a later milestone.
- Reason: M5 already delivered the local backend, auth/authz baseline, and sync API needed for frontend integration; hosted environment strategy is a separate concern that should not block the sync milestone.
- Impact: `T-20260220-09` is marked `outdated`, M5 can close as completed, and future cloud-environment work will be planned independently.
...
- What changed:
  ...
  - `T-20260220-09` was later marked `outdated`; cloud deployment/environment strategy was explicitly deferred to a future milestone rather than blocking M5 closeout.
...
- What remains:
  - Frontend sync integration, sync diagnostics UX, and hosted/cloud environment strategy remain follow-up work outside M5 closeout.
