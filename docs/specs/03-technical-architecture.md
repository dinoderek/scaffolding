...
| M13 sync contract is granular client outbox events with backend idempotent ingest + projection | `Planned` | Backs up full user-owned data while creating an event-level foundation for future low-latency social features. | `docs/specs/milestones/M13-simple-backend-sync.md` |
| Initial mobile sync behavior is auth-gated and non-blocking: one-time bootstrap/merge on enable, then push-dominant periodic sync | `Planned` | Preserves local-first UX while enabling reliable sync convergence without blocking logged-out/offline usage. | `docs/specs/milestones/M13-simple-backend-sync.md` |
